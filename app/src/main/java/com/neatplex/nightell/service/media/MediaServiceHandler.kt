package com.neatplex.nightell.service.media
...
class MediaServiceHandler @Inject constructor
    (private val player: ExoPlayer) : Player.Listener {
        ...
        when(playerEvent){
            ...
            PlayerEvent.PlayPause -> {
                if(player.isPlaying){
                    ...
                    if (_simpleMediaState.value is SimpleMediaState.Completed) {
                        player.seekTo(0)
                        player.play()
                    }
                    else {
    ...
    override fun onPlaybackStateChanged(playbackState: Int) {
        when(playbackState){
            ...
            ExoPlayer.STATE_ENDED -> {
                _simpleMediaState.value = SimpleMediaState.Completed
                stopProgressUpdate()
                player.seekTo(0)
                player.pause()
            }
...
sealed class SimpleMediaState{
    ...
    object Completed : SimpleMediaState()
}